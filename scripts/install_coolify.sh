...
restart_docker_service() {
    echo " - Bypassing Docker restart for WSL2/Docker Desktop environment."
    return 0
}
...
case "$OS_TYPE" in
arch | ubuntu | debian | raspbian | centos | fedora | rhel | ol | rocky | sles | opensuse-leap | opensuse-tumbleweed | almalinux | amzn | alpine | kali) ;;
*)
...
echo " - Bypassing SSH service check for WSL2. Assuming it is running as verified manually."
SSH_DETECTED=true
